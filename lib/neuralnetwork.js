const NeuralNetwork = (function() {
    ...
    class NeuralNetwork {
        ...
        copy() {
            const neuralNetwork = new NeuralNetwork(this.inputNodes, this.layers, this.learningRate);
            for(let i = 0; i < neuralNetwork.layers.length; i++) {
                neuralNetwork.layers[i].weights = this.layers[i].weights;
                neuralNetwork.layers[i].bias = this.layers[i].bias;
            }
